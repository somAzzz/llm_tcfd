...
项目已经在 `src/tcfd_extractor/visualization/` 下实现了完整的 HR 演示报告流水线 (8 个 visualization 模块), 配合 `scripts/` 下的 `build_hr_report.py` (orchestrator) 和 `check_leakage.py` (内嵌隐私检查) 共同组成可执行流水线。 该能力已在 `feat/hr-visualization` 分支完成, 合并 commit 为 `340f750`。 报告本身尚未生成, 也没有上线地址。
...
output/evaluate_cooccurrence/2023/results.jsonl
  └─→ python scripts/build_hr_report.py --output output/hr_report/
       ...
       output/hr_report/{index.html, README.md, .nojekyll}
              ↓
       # Post-build fixup: build_hr_report.py 写出的 README.md
...
| 步骤 | 验证 |
|---|---|
| 1. 跑 `build_hr_report.py` | 退出码 = 0 (内置 `check_leakage.py` 已通过); `index.html` 存在 |
...
- `scripts/build_hr_report.py` 写出的 README 模板硬编码 `tcfd-hr-report`, 本次用 sed 临时修复; 长期方案是把 repo 名作为 CLI 参数传入
