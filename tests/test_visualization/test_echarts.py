...
def test_get_base_option_returns_skeleton():
    ...
    assert opt["textStyle"]["fontFamily"] == "IBM Plex Sans"
...
class TestDarkThemePalette:
    """Portfolio palette: annual-report ink, verdigris signal, cold-blue data."""

    def test_policy_color_uses_bright_blue(self):
        assert TCFD_THEME_CONFIG["colors"]["policy"] == "#7fb7ff"

    def test_market_color_uses_bright_orange(self):
        assert TCFD_THEME_CONFIG["colors"]["market"] == "#e7b75f"

    def test_tech_color_uses_bright_green(self):
        assert TCFD_THEME_CONFIG["colors"]["tech"] == "#36d6b5"

    def test_palette_uses_dark_friendly_bright_colors(self):
        """The three semantic dimensions keep distinct, high-contrast colors."""
        assert TCFD_THEME_CONFIG["colors"]["policy"] == "#7fb7ff"
        assert TCFD_THEME_CONFIG["colors"]["market"] == "#e7b75f"
        assert TCFD_THEME_CONFIG["colors"]["tech"] == "#36d6b5"

    def test_tooltip_text_color_is_white_for_dark(self):
        assert TCFD_THEME_CONFIG["tooltip_style"]["textStyle"]["color"] == "#f2efe8"
...
class TestChartBackgroundMatchesDark:
    ...
    def test_palette_uses_dark_friendly_bright_colors(self):
        """Dark default keeps the three dimension colors legible and distinct."""
        assert TCFD_THEME_CONFIG["colors"]["policy"] == "#7fb7ff"
        assert TCFD_THEME_CONFIG["colors"]["market"] == "#e7b75f"
        assert TCFD_THEME_CONFIG["colors"]["tech"] == "#36d6b5"
...
class TestSunburstOutermostRingDarkBlue:
    ...
    def test_sunburst_dim_colors_use_portfolio_palette(self):
        """Dim colors follow the portfolio palette used by the page."""
        data = [{"name": "Policy", "children": [
            {"name": "Cluster A", "children": [{"name": "kw1", "value": 1}]}
        ]}]
        opt = build_sunburst(data, TCFD_THEME_CONFIG)
        dim_colors = opt["series"][0]["levels"][1]["itemStyle"]["color"]
        assert "#7fb7ff" in dim_colors
        assert "#e7b75f" in dim_colors
        assert "#36d6b5" in dim_colors
