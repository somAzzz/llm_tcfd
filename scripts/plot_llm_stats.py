#!/usr/bin/env python3
"""Plot yearly average llm_sum_class_AB_每万字 for 3 TCFD dimensions.
...
def load_dim_csv(csv_path: Path, dim_name: str) -> pd.DataFrame:
    """Load dimension CSV, compute yearly average llm_sum_class_AB_每万字"""
    df = pd.read_csv(csv_path, encoding="utf-8-sig")
    yearly = df.groupby("year")["llm_sum_class_AB_每万字"].mean().reset_index()
...
def main():
    parser = argparse.ArgumentParser(description="Plot yearly dimension averages (LLM validated)")
    parser.add_argument(
        "--input-dir",
        type=str,
    ...
    parser.add_argument(
        "--output",
        type=str,
        default="output/frequency/llm_dimension_yearly_avg.png",
    ...
    for dim_name, csv_path in dims.items():
        if not csv_path.exists():
            ...
            continue
        df = load_dim_csv(csv_path, dim_name)
        dfs.append(df)
    ...
    ax.set_ylabel("llm_sum_class_AB per 10k chars (yearly avg)", fontsize=12)
    ax.set_title("LLM-Validated TCFD Keyword Co-occurrence Intensity by Year", fontsize=14, fontweight="bold")
